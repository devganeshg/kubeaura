...
#   dist/KubeAura-windows-<arch>.zip
...
ZIP=dist/KubeAura-windows-$ARCH.zip
...
CGO_ENABLED=0 GOOS=windows GOARCH=$ARCH go build -trimpath \
  ...
  -o "$STAGE/KubeAura.exe" ./cmd/kubeaura

# Desktop mode rides on KUBEAURA_DESKTOP=1 (a zip can't ship a shortcut
# with CLI arguments); `start` detaches so the .bat's console flash is brief.
printf '%s\r\n' \
  '@echo off' \
  'set KUBEAURA_DESKTOP=1' \
  'start "" "%~dp0KubeAura.exe"' \
  > "$STAGE/KubeAura.bat"

printf '%s\r\n' \
  "KubeAura $VERSION — Windows" \
  '' \
  'Double-click KubeAura.bat to launch the desktop app.' \
  '' \
  ...
  '' \
  'To pin it: right-click KubeAura.exe > Create shortcut, then open the' \
  'shortcut Properties and append " --desktop" to the Target field.' \
  > "$STAGE/README.txt"

(cd "$STAGE" && zip -qr "../../$ZIP" .)

echo "packaged $ZIP — copy to a Windows machine, unzip, run KubeAura.bat"
